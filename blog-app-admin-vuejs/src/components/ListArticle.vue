...
export default {
  ...
  methods: {
    testApi() {
      ...
      axios.get(`http://localhost:5000/api/Admin/User/${this.Id}/Article?PageNumber=${this.currentPage}`, { headers: headers })
        .then((res) => {
     ...
     DeleteArticle(idArtilce){
      ...
      const headers = {
       ...
       if (window.confirm("Do you really want to delete?")) {
        axios.delete(`http://localhost:5000/api/Admin/User/${this.Id}/Article/${idArtilce}`, { headers: headers })
        .then((res) => {
          this.Reload();
          console(res);
        })
